#!/bin/bash
# Command:
# chmod 700 /vddos/auto-add/vddos-autoadd.sh
...
# Auto recheck/renew ssl again for a domain:
# vddos-autoadd [ssl-again] your-domain.com
...
if [ "$Command" = "list" ]; then
	...
	if [ "$md5sum_latest" != "$md5sum_new" ]; then
		...
		if [ "$SSLmode" != "auto" ]; then
			echo "`cat $listdomains_source | grep . | awk '!x[$0]++' | grep -v '^phpmyadmin.'| grep -v '^roundcube.'| grep -v '^mail.'| grep -v '^webmail.'| grep -v '^www.'`" > $listdomains
		else
...
if [ "$Available" != "" ]; then
	...
	rm -rf /letsencrypt/"$Website".crt
	rm -rf /letsencrypt/"$Website".pri
	rm -rf /vddos/ssl/www."$Website".crt
	rm -rf /vddos/ssl/www."$Website".pri
	rm -rf /letsencrypt/www."$Website".crt
	rm -rf /letsencrypt/www."$Website".pri
	rm -rf /root/.acme.sh/"$Website"_ecc
	rm -rf /root/.acme.sh/www."$Website"_ecc
	echo '- Found: '$Website' Remove all this Web config and ===> Re-request SSL-again!'|tee -a /vddos/auto-add/log.txt
	/usr/bin/vddos-add $Website

	if [ -f /vddos/ssl/"$Website".crt ] && [ -f /root/.acme.sh/"$Website"_ecc/fullchain.cer ] && [ -f /letsencrypt/"$Website".crt ]; then
...
if [ "$SSLmode" = "auto" ]; then
	...
	do
		...
		if [ -f /vddos/ssl/"$Website".crt ] && [ -f /vddos/ssl/"$Website".pri ]; then
			...
			rm -f /vddos/ssl/"$Website".crt
			rm -f /vddos/ssl/"$Website".pri
			cat /root/.acme.sh/"$Website"_ecc/fullchain.cer > /letsencrypt/"$Website".crt
			cat /root/.acme.sh/"$Website"_ecc/"$Website".key > /letsencrypt/"$Website".pri
